...
function ModalBuy({open, close , products}: IProps) {
    ...
    const [totalPrice , setMyTotalPrice] = useState<number | null>(null)
    ...
    const handleSubmitBuy = (values: any)=>{
        ...
        console.log(data)
        // setDiscount(true)
        setMyTotalPrice(totalPrice - (totalPrice * +(data.discount)/ 100))
        }).catch(err=>{
        console.log(err)
        }).finally(()=>{
        // setDiscountValue('')
  ...
  return (
    <Modal size="70%" opened={open} onClose={handleClose}>
        <div className="flex justify-between">
            ...
            <form onSubmit={handleSubmit} className='w-2/5 flex justify-center flex-col items-center gap-6 mt-6'>
                <div
                dir="rtl"
                            ...
                            <div className="flex items-center mt-2 border shadow-md rounded-lg p-2">
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <p className="flex items-center flex-row-reverse justify-end gap-2 text-xs">آپشن: {item.options} </p>
                                        <p className="flex items-center flex-row-reverse justify-end gap-2 text-gray-400 text-xs">گارانتی اصالت و سلامت فیزیکی کالا <BsShieldCheck className="text-black"/></p>
                                        <p className="flex items-center flex-row-reverse justify-end gap-2 text-gray-400 text-xs">ارسال از ۱ روز کاری دیگر<GrDeliver/></p>
                                    </div>
