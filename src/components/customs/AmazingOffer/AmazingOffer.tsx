            ...
            <div className='flex gap-4 items-center'>
                {
                    productFeaturedRandom.slice(0,7).map((item:any)=>(
                        <div style={{backgroundImage:`url(${item.main_image})`}} className='relative bg-[#fefefe] rounded-full w-20 h-20 cursor-pointer bg-cover'>
                            <p className='absolute bottom-1 right-1 bg-[#EF4056] text-white px-2 rounded-3xl'>
                                {(item.final_price * 100 / item.price).toFixed()}%
                            </p>
                        </div>
                    ))
                }
